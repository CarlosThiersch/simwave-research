...
class Middleware:
    ...
    def _exec_fast_gradient(self, c_file, **kwargs):
        """
        Run the adjoint and gradient operator.
        ...
        fast_gradient = lib.fast_gradient
        fast_gradient.restype = ctypes.c_double
        fast_gradient.argtypes = argtypes

        # run the C gradient function
        exec_time = fast_gradient(*args)

        print('Run fast_gradient in %f seconds.' % exec_time)
